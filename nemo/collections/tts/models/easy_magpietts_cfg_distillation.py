...
class _DefaultParams:
    ...
    phonemes_loss_weight: Optional[float] = 0.05

    phonemes_alpha: float = 0.3
...
def _validate_configuration(cfg: DictConfig) -> None:
    ...
    if (
        hasattr(cfg, "phonemes_loss_weight")
        and cfg.get("phonemes_loss_weight") is not None
        and cfg.get("phonemes_loss_weight") < 0
    ):
        raise ValueError("`phonemes_loss_weight` must be non-negative or `None`.")

    if hasattr(cfg, "phonemes_alpha") and not (0 <= cfg.get("phonemes_alpha") <= 1):
        raise ValueError("`phonemes_alpha` must be in the range [0, 1].")
...
class EasyMagpieCFGDistillation(EasyMagpieTTSModel):
    ...
    def _init_losses(self) -> None:
        ...
        if self.distill_phoneme_channel and self.phonemes_loss_weight and self.phonemes_alpha != 1.0:
        ...
        if self.phonemes_loss_weight and self.phonemes_alpha != 0.0:
    ...
    def _compute_loss_helper_codes(
        self,
    ...
    def _compute_loss_helper_phonemes_distillation(
        self,
        teacher_logits: Tensor,
        teacher_codes: Tensor,
        student_logits: Tensor,
        ...
        if self.phonemes_alpha != 1.0:
            ...
            output["kl_loss_phonemes"] = kl_loss

        if self.phonemes_alpha != 0.0:
            ...
            output["ce_loss_phonemes"] = ce_loss

        kl_term = output.get("kl_loss_phonemes", 0.0)
        ce_term = output.get("ce_loss_phonemes", 0.0)
        loss = (1 - self.phonemes_alpha) * kl_term + self.phonemes_alpha * ce_term

        output["loss_phonemes"] = loss

        return output


    def _compute_loss_helper_phonemes_guidance(
        self,
        batch: dict[str, Tensor | list],
        student_logits: Tensor,
        sample_weights: Optional[Tensor],
        ...
        output: dict[str, Tensor] = {}
        target_codes = batch["phoneme_tokens"].long()[:, 1:].unsqueeze(1)
        phoneme_tokens_lens = batch["phoneme_tokens_lens"] - 1
        mask = get_mask_from_lengths(phoneme_tokens_lens)
    ...
    def _compute_loss(
        ...
        output = self._compute_loss_helper_codes(
        ...
        if use_lt:
            lt_output = self._compute_loss_helper_codes(
        ...
        if self.phonemes_loss_weight:
            if self.distill_phoneme_channel:
                phonemes_output = self._compute_loss_helper_phonemes_distillation(
                ...
                phonemes_output = self._compute_loss_helper_phonemes_guidance(
                    batch=batch,
                    student_logits=student_output.logits_phonemes,
                    sample_weights=teacher_output.sample_weights,
                )
            output.update(phonemes_output)
            output["loss"] = output["loss"] + self.phonemes_loss_weight * output["loss_phonemes"]
